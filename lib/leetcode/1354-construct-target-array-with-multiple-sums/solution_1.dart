...
class Solution {
  bool isPossible(List<int> target) {
    if (target.length == 1) return target.first == 1;

    var sum = target.fold(0, (s, e) => s + e);

    final heap = IntMaxHeap.fromList(target);

    while (heap.first > 1) {
      var largest = heap.removeFirst();
      var sec = heap.first;

      do {
        var remain_sum = sum - largest;
        largest -= remain_sum;
        sum = remain_sum + largest;
      } while (largest > sec);
