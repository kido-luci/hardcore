...
class Solution {
  List<int> maxSlidingWindow(List<int> nums, int k) {
    ...
    final heap = MaxHeap<int>.fromList(nums.sublist(0, k));
    final removed = <int, int>{};

    final len = nums.length - k + 1;

    final result = Int32List(len)..[0] = heap.first;

    for (var i = 1; i < len; i++) {
      removed.update(nums[i - 1], (e) => e + 1, ifAbsent: () => 1);
      heap.add(nums[i + k - 1]);
      ...
      // print((nums.sublist(i, i + k), nums[i - 1], nums[i + k - 1], first));
